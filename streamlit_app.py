...
def vytvor_pdf(data, nazev, typ="v"):
    ...
    if os.path.exists("logo.jpg"):
        ...
        pdf.set_x(45) # Posunout text vedle loga
    
    pdf.set_font(p, '', 16)
    pdf.cell(0, 10, KLUB_NAZEV, ln=True)
    pdf.set_font(p, '', 12)
    pdf.set_x(45 if os.path.exists("logo.jpg") else 10)
...
if st.session_state.kolo == 0:
    ...
    for idx, (t1, t2) in enumerate(zap):
        with st.expander(f"Hřiště {idx+1}: {t1} vs {t2}", expanded=True):
            if "VOLNÝ LOS" in (t1, t2):
                ...
                st.info(f"Tým {t1 if t2=='VOLNÝ LOS' else t2} má volný los.")
            else:
                c1, c2 = st.columns(2)
    ...
    st.subheader("Tabulka pořadí")
    st.table(res[["Hráč/Tým", "Výhry", "Skóre +", "Skóre -", "Rozdíl"]])
    
    st.subheader("📊 Přehledná historie turnaje")
